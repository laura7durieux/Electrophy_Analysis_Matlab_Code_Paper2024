...
if GenParams.Session == 'Stress1' | GenParams.Session == 'Stress2'
    GenParams.SubSession = categorical(cellstr(input('What part of the stress protocol do you analyse ? (S1,S2,PS1,PS2)','s')));
else 
...
% Example of channels to used for plot (or later for sleep analysis
dHPCExample = input('Enter a dHPC channel okay as example for plot (5 6 7)');
PRLExample = input('Enter a PRL channel okay as example for plot (14 15 16)');
...
ChParams.dHPCExample = dHPCExample;
ChParams.PRLExample = PRLExample;
...
parpool;
% Filtering the Data - Nelson Totah Filter
...
[LFP_local, localParams, StructuresList]=localRef(ChParams.RatData,ChParams.Structures,LFP_downsampled);

ChParams.localParams = localParams;
ChParams.localStruc = StructuresList;
...
if GenParams.SubSession == 'S1' || GenParams.SubSession == 'S2'
    ...
    % Local
    [DatabyHoursLocal] = slicingSignal(LFP_local,ElectParams.New_SR,CuttingTimes);
    % Not local
    [DatabyHours] = slicingSignal(LFP_downsampled,ElectParams.New_SR,CuttingTimes);
end
% put the cell arrays together
DatabyHours(3,:) = DatabyHoursLocal(2,:);
